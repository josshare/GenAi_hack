...
class ConfigValidator:
    ...
    def _validate_bedrock_config(self, config: Dict[str, Any]) -> None:
        ...
        if not (0 <= temperature <= 1):
            self.result.add_error(f"Bedrock temperature must be between 0 and 1: {temperature}")
        
        # Max tokens validation
        max_tokens = bedrock_config.get('max_tokens', 1000)
        if not isinstance(max_tokens, int) or max_tokens <= 0:
            self.result.add_error(f"Bedrock max_tokens must be positive integer: {max_tokens}")
    ...
    def _validate_agent_config(self, config: Dict[str, Any]) -> None:
        ...
        if not (0 <= confidence_threshold <= 1):
            self.result.add_error(f"Agent confidence_threshold must be between 0 and 1: {confidence_threshold}")
        
        # Max iterations
        max_iterations = agent_config.get('max_iterations', 3)
        if not isinstance(max_iterations, int) or max_iterations <= 0:
            self.result.add_error(f"Agent max_iterations must be positive integer: {max_iterations}")
        ...
        if not isinstance(escalation_timeout, int) or escalation_timeout <= 0:
            self.result.add_error(f"Agent escalation_timeout must be positive integer: {escalation_timeout}")
        ...
        valid_actions = [
            'restart_service', 'scale_up', 'scale_down', 'rollback_deployment',
            'clear_cache', 'restart_database', 'escalate'
        ]
        ...
        for threshold_name, (min_val, max_val) in threshold_configs.items():
            ...
            if value is not None:
                if not isinstance(value, (int, float)) or value < min_val:
                    ...
                    self.result.add_error(f"Invalid {threshold_name}: {value} (max: {max_val})")
    ...
    def _validate_security_config(self, config: Dict[str, Any]) -> None:
        ...
        if security_config.get('encryption_enabled', True):
            ...
            if kms_key and not (kms_key.startswith('arn:aws:kms:') or 
                              kms_key.startswith('alias/') or
                              len(kms_key) == 36):  # UUID format
                self.result.add_warning(f"Potentially invalid KMS key format: {kms_key}")
        
        # IAM settings
        iam_config = security_config.get('iam', {})
        if iam_config.get('create_roles', True):
            role_prefix = iam_config.get('role_prefix', 'ai-agent')
            if not role_prefix or not role_prefix.replace('-', '').replace('_', '').isalnum():
    ...
    def _validate_env_file(self) -> None:
        ...
        try:
            ...
            if missing_vars:
                self.result.add_warning(f"Missing environment variables: {', '.join(missing_vars)}")
        ...
        try:
            ...
            if bedrock_config:
                try:
                    ...
                    # This doesn't make an actual call, just validates client creation
